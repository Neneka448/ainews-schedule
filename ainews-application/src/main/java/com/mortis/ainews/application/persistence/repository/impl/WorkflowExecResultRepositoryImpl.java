...
@Slf4j
public class WorkflowExecResultRepositoryImpl implements IWorkflowExecResultRepository {
    ...
    public WorkflowExecResultDO save(WorkflowExecResultDO workflowExecResult) {
        log.debug("Saving workflow execution result for schedule ID: {}", workflowExecResult.getScheduleId());
    ...
    public Optional<WorkflowExecResultDO> findById(Long id) {
        log.debug("Finding workflow execution result by ID: {}", id);
        
        return workflowExecResultRepository.findById(id)
    ...
    public List<WorkflowExecResultDO> findByScheduleId(Long scheduleId) {
        log.debug("Finding workflow execution results for schedule ID: {}", scheduleId);
    ...
    public PageData<WorkflowExecResultDO> findByScheduleIdWithPaging(Long scheduleId, PageQuery pageQuery) {
        log.debug("Finding workflow execution results for schedule ID: {} with pagination", scheduleId);
        
        Pageable pageable = PageRequest.of(
                pageQuery.getPageNum() - 1, // Spring Data starts from 0
                pageQuery.getPageSize(),
                Sort.by(Sort.Direction.DESC, "createdAt") // Most recent first
        );
        ...
        List<WorkflowExecResult> filteredResults = page.getContent().stream()
                .filter(result -> result.getScheduleId().equals(scheduleId))
                .toList();
        ...
        return new PageData<>(
                ...
                (long) filteredResults.size(), // This is not accurate for total count
                pageQuery
        );
    }
    ...
    public List<WorkflowExecResultDO> findByStatus(String status) {
        log.debug("Finding workflow execution results by status: {}", status);
    ...
    public List<WorkflowExecResultDO> findByActivityType(String activityType) {
        log.debug("Finding workflow execution results by activity type: {}", activityType);
    ...
    public Optional<WorkflowExecResultDO> findLatestByScheduleId(Long scheduleId) {
        log.debug("Finding latest workflow execution result for schedule ID: {}", scheduleId);
        
        return workflowExecResultRepository.findFirstByScheduleIdOrderByCreatedAtDesc(scheduleId)
                .map(converterFacade.workflowExecResultConverter::toDO);
    }
    ...
    public List<WorkflowExecResultDO> findByCreatedAtAfter(Instant createdAt) {
        log.debug("Finding workflow execution results created after: {}", createdAt);
    ...
    public List<WorkflowExecResultDO> findByScheduleIdAndStatus(Long scheduleId, String status) {
        log.debug("Finding workflow execution results for schedule ID: {} and status: {}", scheduleId, status);
        
        List<WorkflowExecResult> pos = workflowExecResultRepository.findByScheduleIdAndStatus(scheduleId, status);
        return converterFacade.workflowExecResultConverter.toDOs(pos);
    }
    ...
    public List<WorkflowExecResultDO> findByScheduleIdAndActivityType(Long scheduleId, String activityType) {
        log.debug("Finding workflow execution results for schedule ID: {} and activity type: {}", scheduleId, activityType);
        
        List<WorkflowExecResult> pos = workflowExecResultRepository.findByScheduleIdAndActivityType(scheduleId, activityType);
        return converterFacade.workflowExecResultConverter.toDOs(pos);
    }
    ...
    public void deleteById(Long id) {
        log.debug("Deleting workflow execution result by ID: {}", id);
    ...
    public long countByScheduleId(Long scheduleId) {
        log.debug("Counting workflow execution results for schedule ID: {}", scheduleId);
        
        return workflowExecResultRepository.findByScheduleId(scheduleId).size();
    }

    @Override
    @Transactional(readOnly = true)
    public long countByStatus(String status) {
        log.debug("Counting workflow execution results by status: {}", status);
        
        return workflowExecResultRepository.findByStatus(status).size();
    }
}
