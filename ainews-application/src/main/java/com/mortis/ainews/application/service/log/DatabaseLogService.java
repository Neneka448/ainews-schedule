...
public class DatabaseLogService {
    ...
    private static final int MAX_MESSAGE_LENGTH = 200_000; // 约 200KB，避免超大日志
    ...
    public void storeLogAsync(
        String level, String message, LogTypeEnum logType, Long belongId, Map<String, Object> extraData)
    {
        try {
            // Resolve belong_id using LogHelper
            Long resolvedBelongId = logHelper.resolveBelongId(
                logType,
                belongId
            );

            // Parse and validate extra data
            Map<String, Object> processedExtra = logHelper.parseExtraData(
                logType,
                extraData
            );

            // Truncate long message to avoid DB errors
            String safeMessage = truncateIfTooLong(message);

            // Create and save log entry
            AiNewsLog logEntry = new AiNewsLog(
                level,
                logType,
                safeMessage,
                resolvedBelongId,
            ...
            log.error(
                "Failed to store log to database: level={}, message={}, type={}",
                level,
                message,
                logType,
                e
        ...
        String level, String message, LogTypeEnum logType, Long belongId, Map<String, Object> extraData)
    {
        try {
            ...
            log.error(
                "Failed to store log to database synchronously: level={}, message={}, type={}",
                level,
                message,
                logType,
                e
     ...
     */
    @Async
    public void storeLogWithContext(
        String level, String message, LogTypeEnum logType,
        Long entityId, Map<String, Object> additionalContext)
    {
        LogHelper.LogContext context = logHelper.createLogContext(
            logType,
            entityId,
            additionalContext
        );
